...
class _Tensor(Structure):
    ...
    ("shapes", POINTER(c_int)),
    ("information", c_int)
    ]

lib.tensor_create_nonstruct.argtypes = (POINTER(c_float), c_int, POINTER(c_int), c_int)
lib.tensor_create_nonstruct.restype = POINTER(_Tensor)
lib.tensor_create_nonstruct_deep.argtypes = (POINTER(c_float), c_int, POINTER(c_int), c_int)
...
def _numpy2Tensor(nparray):
    return lib.tensor_create_nonstruct(nparray.ctypes.data_as(POINTER(c_float)), nparray.size, nparray.ctypes.shape_as(c_int), nparray.ndim)
