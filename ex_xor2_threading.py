...
for i in range(10000):
    ...
    for layer in forwards:
        t1 = threading.Thread(target=layer.forward, args = (0,2))
        t2 = threading.Thread(target=layer.forward, args = (1,2))
        t1.start()
        t2.start()
        t1.join()
        t2.join()
    ...
    for layer in backwards:
        t1 = threading.Thread(target=layer.backward, args=(0,2))
        t2 = threading.Thread(target=layer.backward, args=(1,2))
        t1.start()
        t2.start()
        t1.join()
        t2.join()
    ...
    for layer in forwards:
        t1 = threading.Thread(target=layer.update, args=(optimizer, 0,2))
        t2 = threading.Thread(target=layer.update, args=(optimizer, 1,2))
        t1.start()
        t2.start()
        t1.join()
        t2.join()
