...
void cnn_ada_callback_update(struct cnn_Optimizer *optimizer, struct cnn_UpdateList *list, int index, int max_index){
...
void cnn_ada_callback_initUpdate(struct cnn_UpdateList *list){
    for(int set_index = 0; set_index < list->setSize; set_index++){
        if(list->sets[set_index]->momnt == 0){
            ...
            if(list->sets[set_index]->momnt->size != list->sets[set_index]->value->size){
                tensor_release_deep(list->sets[set_index]->momnt);
                list->sets[set_index]->momnt = tensor_create_values_deep(list->sets[set_index]->value->shapes, list->sets[set_index]->value->dim,0);
            }
        }
    }
...
struct cnn_Optimizer *cnn_create_Ada(float learning_rate){
    return cnn_create_optimizer(learning_rate, cnn_ada_callback_update, cnn_ada_callback_initUpdate);
}
