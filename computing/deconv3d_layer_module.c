...
struct Tensor *cnn_create_deconv3d_layer_out(struct Tensor *x, struct Tensor *filter, int stride, int pad){
    long long shape[4] = {x->shapes[0], filter->shapes[0], stride * (x->shapes[2] - 1) + filter->shapes[2] - 2 * pad, stride * (x->shapes[3] - 1) + filter->shapes[3] - 2 * pad};
    return tensor_create_values_deep(shape, 4, 0);
}

void cnn_comput_deconv3d_layer_forward(struct Tensor *x, struct Tensor *filter, struct Tensor *bias, int stride, int pad, struct Tensor *out, int index, int max_index){
    // conv에서 backward 코드를 활용함.
    ...
    for(int out_index = index * out->size / max_index, out_index_max = (index + 1) * out->size / max_index; out_index < out_index_max; out_index ++){
        ...
        int out3 = out_index % out->shapes[3];
        //dx3 = dx_index % dx_shape[3]
        int out2 = out_index / out->shapes[3] % out->shapes[2];
        //dx2 = dx_index // dx_shape[3] % dx_shape[2]
        float tmp = 0; // 이녀석 위치 연구가 필요함.

        int out3pad = out3 + pad;
        //dx3pad = dx3 + pad
        int out2pad = out2 + pad;
        //dx2pad = dx2 + pad

        int x_index0 = out_index / multipler_out1 * multipler_x1;
        //dout_index0 = (dx_index // multipler_dx1) * multipler_dout1
        int filter_index1 = ((out_index / multipler_out2) % out->shapes[1]) * multipler_filter2;
        ...
        for(int x1 = 0, x1_max = x->shapes[1]; x1 < x1_max; x1++){
            //for dout1 in dout1_range:
            int x_index1 = x_index0 + x1 * multipler_x2;
            //dout_index1 = dout_index0 + dout1 * multipler_dout2
            int filter_index0 = filter_index1 + x1 * multipler_filter1;
            //filter_index0 = filter_index1 + dout1 * multipler_filter1

            for(int x2 = MAX((out2pad - filter->shapes[2]) / stride, -1) + 1, x2_max = MIN(-(-(out2pad + 1) / stride), x->shapes[2]); x2 < x2_max; x2++){
                //for dout2 in dout2_range:
                int x_index2 = x_index1 + x2 * x->shapes[3];
                //dout_index2 = dout_index1 + dout2 * dout_shape[3]
                int filter_index2 = filter_index0 + (out2pad - x2 * stride) * filter->shapes[3];
                //filter_index2 = filter_index0 + (dx2pad - dout2 * stride) * filter_shape[3]
                for(int x3 = MAX((out3pad - filter->shapes[3]) / stride, -1) + 1, x3_max = MIN(-(-(out3pad + 1) / stride), x->shapes[3]); x3 < x3_max; x3++){
                    //for dout3 in dout3_range:
                    tmp += x->scalas[x_index2 + x3] * filter->scalas[filter_index2 + out3pad - x3 * stride];
                }
            }
        }
...
void cnn_comput_deconv3d_layer_backward(struct Tensor *dout, struct Tensor *filter, int stride, int pad, int padding, struct Tensor *dx, int index, int max_index){
    int multipler_out2 = dx->shapes[3] * dx->shapes[2];
    //multipler_out2 = out_shape[3] * out_shape[2]
    int multipler_out1 = multipler_out2 * dx->shapes[1];
    //multipler_out1 = multipler_out2 * out_shape[1]
    int multipler_filter2 = filter->shapes[3] * filter->shapes[2];
    //multipler_filter2 = filter_shape[3] * filter_shape[2]
    int multipler_filter1 = multipler_filter2 * filter->shapes[1];
    //multipler_filter1 = multipler_filter2 * filter_shape[1]
    int multipler_x2 = dout->shapes[3] * dout->shapes[2];
    //multipler_x2 = x_shape[3] * x_shape[2]
    int multipler_x1 = multipler_x2 * dout->shapes[1];
    ...
    for (int out_index = index * dx->size / max_index, out_index_max = (index + 1) * dx->size / max_index; out_index < out_index_max; out_index++){
        //for out_index in range(index * len(out_array) // max_index, (index + 1) * len(out_array) // max_index):
        int o0 = out_index / multipler_out1 % dx->shapes[0];
        //o0 = out_index // multipler_out1 % out_shape[0]
        int filter_index0 = (out_index / multipler_out2 % dx->shapes[1]) * multipler_filter1;
        //filter_index0 = (out_index // multipler_out2 % out_shape[1]) * multipler_filter1
        int x2_tmp = (out_index / dx->shapes[3] % dx->shapes[2]) * stride - pad;
        //x2_tmp = (out_index // out_shape[3] % out_shape[2]) * stride - pad
        int x3_tmp = (out_index % dx->shapes[3]) * stride - pad;
        ...
        for(int f1 = 0, f1_max = filter->shapes[1]; f1 < f1_max; f1++){
            ...
            for(int f2 = 0, f2_max = filter->shapes[2]; f2 < f2_max; f2++){
                ...
                int x2 = f2 + x2_tmp;
                //x2 = f2 + x2_tmp
                int x_index2 = x_index1 + x2 * dout->shapes[3];
                //x_index2 = x_index1 + x2 * x_shape[3]
                if((-1 < x2) && (x2 < dout->shapes[2])){
                    //if((-1 < x2) & (x2 < x_shape[2])):
                    for(int f3 = 0, f3_max = filter->shapes[3]; f3 < f3_max; f3++){
                        //for f3 in f3_range:
                        int x3 = f3 + x3_tmp;
                        //x3 = f3 + x3_tmp
                        if((-1 < x3) && (x3 < dout->shapes[3]))
                            tmp += dout->scalas[x_index2 + x3] * filter->scalas[filter_index2 + f3];
                        else
                            tmp += padding;
                    }
                }
        ...
        dx->scalas[out_index] = tmp;
    }
}
void cnn_comput_deconv3d_layer_dfilter(struct Tensor *dout, struct Tensor *x, int stride, int pad, int padding, struct Tensor *dfilter, int index, int max_index){
...
void cnn_comput_deconv3d_layer_dbias(struct Tensor *dout, struct Tensor *dbias, int index, int max_index){
